...
<head>
  ...
  <style>
    ...
    .suspicious { color: #d73a49; font-weight: bold; }
    .benign { color: #28a745; font-weight: bold; }
    .inconclusive { color: #6a737d; font-weight: bold; }
  </style>
</head>
<body>
  <h1>${SECTION_TITLE}</h1>
...
# Determine whether raw output is suspicious, benign, or inconclusive.
# Parameters:
#   $1 = technique identifier (for special-case logic)
#   $2 = path to raw output file
# Returns via global variables:
#   SUSPICIOUS = YES / BENIGN / INCONCLUSIVE
...
apply_heuristic() {
  ...
  case "$TECH_ID" in
    kernel_taint)
      if [ ! -r "$RAW_OUT" ]; then
        SUSPICIOUS="INCONCLUSIVE"
        REASON="Cannot read tainted file"
      else
        TAINT_VAL=$(grep -E '^[0-9]+' "$RAW_OUT" 2>/dev/null)
        if [ "$TAINT_VAL" = "0" ]; then
          SUSPICIOUS="BENIGN"
          REASON="Kernel not tainted (0)"
        else
          SUSPICIOUS="YES"
          REASON="Kernel tainted (value: $TAINT_VAL)"
        fi
      fi
      ;;
    lsmod_list)
      if [ ! -s "$RAW_OUT" ]; then
        SUSPICIOUS="INCONCLUSIVE"
        REASON="lsmod returned empty"
      else
        SUSPICIOUS="BENIGN"
        REASON="Modules loaded normally"
      fi
      ;;
    compare_lsmod_sys_module)
      if grep -q '^[-+] ' "$RAW_OUT"; then
        SUSPICIOUS="YES"
        REASON="Mismatch between lsmod and /sys/module"
      else
        SUSPICIOUS="BENIGN"
        REASON="lsmod and /sys/module match"
      fi
      ;;
    detect_hidden_modules)
      if grep -q '^[-+] ' "$RAW_OUT"; then
        SUSPICIOUS="YES"
        REASON="Potential hidden/malicious module"
      else
        SUSPICIOUS="BENIGN"
        REASON="No hidden modules detected"
      fi
      ;;
    deleted_binaries)
      if [ -s "$RAW_OUT" ]; then
        SUSPICIOUS="YES"
        REASON="Deleted-binary processes present"
      else
        SUSPICIOUS="BENIGN"
        REASON="No deleted-binary processes"
      fi
      ;;
    *)
      if [ ! -f "$RAW_OUT" ]; then
...
# Run a check, capture raw output, then apply the appropriate heuristic
run_check() {
  TECH_ID="$1"         # e.g., "kernel_taint" or "deleted_binaries"
  TECH_DESC="$2"       # human-readable technique description
  TTP="$3"             # MITRE ATT&CK TTP
  CMD="$4"             # command to execute
  RAW_OUT="$5"         # path to raw output file
  ...
  if [ "$SKIP_UTIL" -eq 1 ]; then
    ...
    SUSPICIOUS="INCONCLUSIVE"
    REASON="Utility not present"
  else
    sh -c "$CMD" > "$RAW_OUT" 2>&1
    RC=$?
    if [ $RC -ne 0 ]; then
      if echo "$CMD" | grep -q "grep" && [ $RC -eq 1 ]; then
        echo "" > "$RAW_OUT"
      else
        echo "# Error (exit code $RC)" >> "$RAW_OUT"
        SUSPICIOUS="INCONCLUSIVE"
        if grep -qi "Permission denied" "$RAW_OUT"; then
  ...
  apply_heuristic "$TECH_ID" "$RAW_OUT"

  case "$SUSPICIOUS" in
    YES)
      echo "<p class=\"suspicious\">Suspicious: ${REASON}</p>" >> "$HTML_FILE"
      echo "Technique: ${TECH_DESC} | MITRE ATT&CK TTP: ${TTP}"
      echo "Result: SUSPICIOUS – ${REASON}"
      ;;
    BENIGN)
      echo "<p class=\"benign\">Suspicious: NO (${REASON})</p>" >> "$HTML_FILE"
      echo "Technique: ${TECH_DESC} | MITRE ATT&CK TTP: ${TTP}"
      echo "Result: BENIGN – ${REASON}"
      ;;
    INCONCLUSIVE)
      echo "<p class=\"inconclusive\">Suspicious: INCONCLUSIVE (${REASON})</p>" >> "$HTML_FILE"
      echo "Technique: ${TECH_DESC} | MITRE ATT&CK TTP: ${TTP}"
      echo "Result: INCONCLUSIVE – ${REASON}"
      ;;
  esac
}
...
###############################################################################
###  Section Scans with Refined Heuristics
###############################################################################
...
scan_kernel_modules() {
  ...
  run_check "lsmod_list" "List loaded kernel modules" "T1215" "lsmod" "$RAW_OUT"

  # 1.2 Check for kernel taint
  RAW_OUT="${SECTION_DIR}/02_kernel_tainted.txt"
  run_check "kernel_taint" "Check for kernel taint" "T1215" "cat /proc/sys/kernel/tainted" "$RAW_OUT"
  ...
  '
  run_check "compare_lsmod_sys_module" "Compare lsmod vs /sys/module" "T1215" "$CMD" "$RAW_OUT"
  ...
  CMD='
    ...
    diff -u '"${SECTION_DIR}/tmp1.lst"' '"${SECTION_DIR}/tmp2.lst"'
  '
  run_check "detect_hidden_modules" "Detect hidden/malicious modules" "T1215" "$CMD" "$RAW_OUT"
  rm -f "${SECTION_DIR}/tmp1.lst" "${SECTION_DIR}/tmp2.lst"
...
scan_proc_artifacts() {
  ...
  run_check "deleted_binaries" "Processes running deleted binaries" "T1055" "ls -alR /proc/*/exe 2>/dev/null | grep deleted" "$RAW_OUT"

  # 2.2 Memory-only (‘memfd’) file descriptors
  RAW_OUT="${SECTION_DIR}/02_memfd_fds.txt"
  CMD='for pid in $(ls /proc 2>/dev/null | grep "^[0-9]\+$"); do
        ls "/proc/${pid}/fd" 2>/dev/null | grep memfd && echo "PID: ${pid}"
      done'
  run_check "" "Memory-only (‘memfd’) file descriptors" "T1055" "$CMD" "$RAW_OUT"

  # 2.3 Deleted mappings in process memory
...
scan_filesystem_integrity() {
  ...
  run_check "" "Verify installed RPM files" "T1105" "rpm -Va 2>/dev/null | grep '^..5\\.'" "$RAW_OUT"

  # 3.2 Verify installed DEB files
  RAW_OUT="${SECTION_DIR}/02_deb_verify.txt"
  run_check "" "Verify installed DEB files" "T1105" "debsums -c 2>/dev/null" "$RAW_OUT"
...
scan_dfir_tools() {
  ...
  if [ "$SKIP_UTIL" -eq 1 ]; then
    ...
    echo "Result: INCONCLUSIVE – Utility not present"
    ...
    echo "<p class=\"benign\">Suspicious: NO (Strace ran successfully on PID 1)</p>" >> "$HTML_FILE"
    echo "Technique: Strace on process | MITRE ATT&CK TTP: T1055"
    echo "Result: BENIGN – Strace ran successfully"
  fi
